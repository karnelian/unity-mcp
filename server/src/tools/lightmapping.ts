...
export function registerLightmappingTools(server: McpServer, bridge: UnityBridge) {
  ...
  server.tool("unity_lightmapping_bake", "Start lightmap baking.", {
  ...
  server.tool("unity_lightmapping_cancel", "Cancel ongoing lightmap baking.", {}, async (p) => {
    const r = await bridge.request("lightmapping.cancel", p);
    return { content: [{ type: "text", text: JSON.stringify(r, null, 2) }] };
  });

  server.tool("unity_lightmapping_clear", "Clear baked lightmap data.", {}, async (p) => {
    const r = await bridge.request("lightmapping.clear", p);
    return { content: [{ type: "text", text: JSON.stringify(r, null, 2) }] };
  });

  server.tool("unity_lightmapping_getSettings", "Get lightmap baking settings.", {}, async (p) => {
    const r = await bridge.request("lightmapping.getSettings", p);
    return { content: [{ type: "text", text: JSON.stringify(r, null, 2) }] };
  });

  server.tool("unity_lightmapping_setSettings", "Set lightmap baking settings.", {
  ...
  server.tool("unity_lightmapping_getProgress", "Get current baking progress.", {}, async (p) => {
